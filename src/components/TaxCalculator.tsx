...
import { PiggyBank, ShieldCheck, HelpCircle, ArrowRight, TrendingUp, DollarSign, Info } from 'lucide-react';
...
export function TaxCalculator() {
  ...
  const totalDividends = avgPortfolioValue * (dividendYield / 100) * investmentYears;
  const totalCapitalGains = capitalGainsYearly * investmentYears;
  const totalGrossProfit = totalDividends + totalCapitalGains;

  // 1. 일반 계좌 (Taxable)
  // 배당세 15.4%
  const taxableDividendTax = totalDividends * 0.154;
  ...
  // 만기 연금 전환 추가 세액공제 (전환금액의 10%, 최대 300만원)
  ...
  // 과세이연: 운용기간 중 세금 0원
  // 연금 수령 시 저율 연금소득세 (55세 이후 인출 시 평균 4.4% 적용)
  ...
  const chartData = Array.from({ length: 5 }, (_, i) => {
    ...
    const div = avgVal * (dividendYield / 100) * yr;
    const gain = capitalGainsYearly * yr;
    const gross = div + gain;

    // Normal
    const normTax = div * 0.154;
          ...
          <div className="bg-white rounded-2xl p-6 shadow-xs border border-slate-200 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-slate-400 font-medium">기본</span>
              </div>
              <h4 className="font-extrabold text-lg text-slate-900 mb-1">일반 계좌</h4>
              <p className="text-xs text-slate-500 mb-4">배당금 받을 때마다 15.4% 원천징수</p>
              ...
              <div className="space-y-2.5 py-3 border-t border-slate-100 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-500">배당소득세 (15.4%)</span>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-xs border-2 border-indigo-500 relative flex flex-col justify-between">
            ...
            <div>
              ...
              <div className="space-y-2.5 py-3 border-t border-slate-100 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-500">인출시 연금소득세(4.4%)</span>
          ...
          <table className="w-full text-left text-xs min-w-[600px]">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              ...
              <tr>
                ...
                <td className="p-3.5 text-rose-600 font-bold">15.4% (원천징수)</td>
                <td className="p-3.5 font-bold text-emerald-700">비과세 + 9.9% (분리과세)</td>
                <td className="p-3.5 font-bold text-indigo-700">0% (과세이연 후 인출시 3.3~5.5%)</td>
              </tr>
              <tr>
